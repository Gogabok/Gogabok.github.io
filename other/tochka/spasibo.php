<?php
$imail="info@tochkaresheny.ru"; //Email (можно через запятую - email1, email2, email3)
...
// сообщение не отправилось
  ...
  if (mail($imail, $title, $message, $header) == 1) {
    mail("quiz24-job@yandex.ru", $title, $message, $header);
    mail("sergeykozin1@gmail.com", $title, $message, $header);
    mail("tochkalidy@yandex.ru", $title, $message, $header);
//Успешная заявка 
  ?> 
