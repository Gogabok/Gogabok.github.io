...
$(document).ready(function() {
	$('#form').submit(function() { // проверка на пустоту заполненных полей. Атрибут html5 — required не подходит (не поддерживается Safari)
		...
		$.ajax({
			...
			ym(53482036, 'reachGoal', 'fast_payments');
...
$(document).ready(function() {
	$('#form1').submit(function() { // проверка на пустоту заполненных полей. Атрибут html5 — required не подходит (не поддерживается Safari)
		...
		$.ajax({
			...
			ym(53482036, 'reachGoal', 'everyweek_payments');
			$('.js-overlay-thank-you').fadeIn();
			$(this).find('input').val('');
			$('#form1').trigger('reset');
		});
...
$(document).ready(function() {
	$('#form5').submit(function() { // проверка на пустоту заполненных полей. Атрибут html5 — required не подходит (не поддерживается Safari)
		if (document.form5.name.value == '' || document.form5.phone.value == '' ) {
			valid = false;
			return valid;
		}
		$.ajax({
