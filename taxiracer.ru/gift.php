...
if (empty($_POST["fname"]) or empty($_POST["name"]) or empty($_POST["tel"]) or empty($_POST["mail"]) or empty($_POST["city"]) or $_POST["city"] == "")
	{
	$text = "Вы не заполнили
	обязательные поля или
	не указали город в
	котором хотите работать!";
	$text2 = "Попробуйте еще раз!";
	}
else {
		if ($_POST["city"] == "1") {$city = "Санкт-Петербург";}
		if ($_POST["city"] == "2") {$city = "Москва";}
		if ($_POST["city"] == "3") {$city = "Калининград";}
		...
		$mail_to = "fms@albinast.ru";
		$Subject = "Заявка с сайта TaxiRacer.Ru";
		...
		$Body = "";
		$Body .= "Заявка с сайта TaxiRacer.Ru";
		$Body .= "<br/>";
		$Body .= "<strong>Город работы: </strong>";
		$Body .= $city;
		$Body .= "<br/>";
		$Body .= "<strong>Фамилия: </strong>";
		$Body .= $_POST["fname"];
		$Body .= "<br/>";
		...
		$Body .= $_POST["tel"];
		$Body .= "<br/>";
		$Body .= "<strong>Почта: </strong>";
		$Body .= $_POST["mail"];
		$Body .= "<br/>";
		$Body .= "<strong>Отправил: </strong>";
		$Body .= $_POST["youname"];
		$Body .= "<br/>";
		$Body .= "<strong>Почта отправившего: </strong>";
		$Body .= $_POST["youtel"];
